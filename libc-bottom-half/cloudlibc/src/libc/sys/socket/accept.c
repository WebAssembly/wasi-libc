...
int tcp_accept(tcp_socket_t* socket, bool client_blocking, struct sockaddr* addr, socklen_t* addrlen) {
...
int udp_accept(udp_socket_t* socket, bool client_blocking, struct sockaddr* addr, socklen_t* addrlen) {
...
int accept4(int socket, struct sockaddr *restrict addr, socklen_t *restrict addrlen, int flags) {
    ...
    case DESCRIPTOR_TABLE_ENTRY_TCP_SOCKET:
        return tcp_accept(&entry->tcp_socket, client_blocking, addr, addrlen);
    case DESCRIPTOR_TABLE_ENTRY_UDP_SOCKET:
        return udp_accept(&entry->udp_socket, client_blocking, addr, addrlen);
    default:
        errno = EOPNOTSUPP;
        return -1;
    }
