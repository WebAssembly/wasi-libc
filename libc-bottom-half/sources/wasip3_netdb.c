...
static int map_error(ip_name_lookup_error_code_t error) {
  switch (error) {
  case IP_NAME_LOOKUP_ERROR_CODE_ACCESS_DENIED: return EACCES;
  case IP_NAME_LOOKUP_ERROR_CODE_INVALID_ARGUMENT: return EINVAL;
  case IP_NAME_LOOKUP_ERROR_CODE_NAME_UNRESOLVABLE: return EAI_NONAME;
  case IP_NAME_LOOKUP_ERROR_CODE_TEMPORARY_RESOLVER_FAILURE: return EAI_AGAIN;
  case IP_NAME_LOOKUP_ERROR_CODE_PERMANENT_RESOLVER_FAILURE: return EAI_FAIL;
  default:
...
static int add_addr(ip_name_lookup_ip_address_t address, in_port_t port,
...
int getaddrinfo(const char *restrict host, const char *restrict serv,
  ...
  *res = NULL;
  ...
  wasip3_string_t name = {.ptr = (uint8_t *)host, .len = strlen(host)};
  ip_name_lookup_result_list_ip_address_error_code_t result;
  wasip3_subtask_status_t status = ip_name_lookup_resolve_addresses(name, &result);
  wasip3_subtask_block_on(status);
  if (result.is_err) {
    return map_error(result.val.err);
  }
  ...
  for (size_t i = 0; i < result.val.ok.len && ret == 0; ++i) {
    ip_name_lookup_ip_address_t address = result.val.ok.ptr[i];
