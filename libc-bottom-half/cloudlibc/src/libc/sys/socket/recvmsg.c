...
static_assert(MSG_CTRUNC == __WASI_SOCK_RECV_FDS_TRUNCATED, "Value mismatch");
static_assert(MSG_TRUNC == __WASI_SOCK_RECV_DATA_TRUNCATED, "Value mismatch");
...
ssize_t recvmsg(int socket, struct msghdr *message, int flags) {
  ...
  __wasi_recv_in_t ri = {
      .ri_data = (const __wasi_iovec_t *)message->msg_iov,
      .ri_data_len = message->msg_iovlen,
      .ri_fds = (__wasi_fd_t *)CMSG_DATA(fds_cmsg),
      .ri_fds_len =
