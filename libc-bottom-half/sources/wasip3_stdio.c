...
static int stdio3_get_write_stream(void *data,
                                 filesystem_stream_u8_t *out_stream,
                                 off_t **out_offset) {
  stdio3_t *stdio = (stdio3_t *)data;
  *out_stream = (filesystem_stream_u8_t)stdio->stdout;
  *out_offset = NULL;
  return 0;
}
...
int stdio_add_input(int fd, stdin_stream_u8_t stream, stdin_future_result_void_error_code_t future)
{
...
int stdio3_add_output(int fd, wasip3_subtask_status_t (*func)(stdin_stream_u8_t data, stdout_result_void_error_code_t *result)) {
    stdio3_t *stdio = calloc(1, sizeof(stdio3_t));
    if (!stdio) {
        errno = ENOMEM;
        return -1;
...
int __wasilibc_init_stdio() {
    ...
    fd = stdio3_add_output(2, (wasip3_subtask_status_t (*)(stdin_stream_u8_t,
