...
typedef struct preopen {
    ...
    wasip2_string_t prefix;

    /// The file descriptor.
    filesystem_preopens_own_descriptor_t fd;
} preopen;
...
static int internal_register_preopened_fd_unlocked(filesystem_preopens_own_descriptor_t fd,
    ...
    preopens[num_preopens++] = (preopen) { relprefix, fd, };
...
static int internal_register_preopened_fd(filesystem_preopens_own_descriptor_t fd,
                                          wasip2_string_t relprefix) {
    LOCK(lock);
...
int __wasilibc_register_preopened_fd(filesystem_preopens_own_descriptor_t fd,
    ...
    return internal_register_preopened_fd(fd, prefix);
}
...
int __wasilibc_find_abspath(const char *path,
    ...
    filesystem_preopens_own_descriptor_t handle;
    LOCK(lock);
    for (size_t i = num_preopens; i > 0; --i) {
        const preopen *pre = &preopens[i - 1];
        const uint8_t *prefix = pre->prefix.ptr;
        size_t len = pre->prefix.len;
        ...
        if ((!found_handle || len > match_len) &&
            prefix_matches(prefix, len, path))
        {
            found_handle = true;
            handle = pre->fd;
            match_len = len;
            *abs_prefix = (const char*) prefix;
        }
    }
    UNLOCK(lock);
    ...
    *relative_path = computed;
    ...
    entry.file.file_handle = filesystem_borrow_descriptor(handle);
...
void __wasilibc_populate_preopens(void) {
    ...
    for (size_t i = 0; i < preopens.len; ++i) {
        ...
        if (internal_register_preopened_fd_unlocked(name_and_descriptor.f0,
                                                    name_and_descriptor.f1)) {
            goto software;
        }
    }
...
void __wasilibc_reset_preopens(void) {
    ...
    if (num_preopens) {
        for (int i = 0; i < num_preopens; ++i) {
            wasip2_string_free(&preopens[i].prefix);
        }
        free(preopens);
    }
