    ...
    struct sockaddr* addr , socklen_t* addrlen)
{
    ...
    udp_borrow_incoming_datagram_stream_t incoming_borrow = udp_borrow_incoming_datagram_stream(streams.incoming);
    while (true) {
        udp_list_incoming_datagram_t datagrams;
        if (!udp_method_incoming_datagram_stream_receive(incoming_borrow, 1, &datagrams, &error)) {
            errno = __wasi_sockets_utils__map_error(error);
            return -1;
        }
        ...
        if (datagrams.len) {
            ...
            if (output_addr.tag != OUTPUT_SOCKADDR_NULL) {
                __wasi_sockets_utils__output_addr_write(datagram.remote_address, &output_addr);
            }
            ...
            poll_borrow_pollable_t pollable_borrow = poll_borrow_pollable(streams.incoming_pollable);
            poll_method_pollable_block(pollable_borrow);
        } else {
            errno = EWOULDBLOCK;
            return -1;
