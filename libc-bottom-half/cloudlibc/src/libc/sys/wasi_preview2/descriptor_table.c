...
__attribute__((__import_module__("wasi_snapshot_preview1"), __import_name__("adapter_open_badfd"))) extern int32_t __wasi_preview1_adapter_open_badfd(int32_t);
...
static descriptor_table_t global_table = { .entries = NULL, .mask = 0, .used = 0 };
...
static bool remove(int fd, descriptor_table_variant_t* variant, descriptor_table_t* table)
{
    ...
    if (e->occupied) {
        *variant = e->variant;
        ...
        // interrupted linear probe) if we left this one unoccupied and move
        // them as necessary.
        i = i & table->mask;
        size_t j = i;
        while (true) {
        ...
        // If the load factor has dropped below 25%, shrink the table to reduce
        // memory footprint.
