    ...
    const struct sockaddr* addr, socklen_t addrlen)
{
    ...
    case UDP_SOCKET_STATE_UNBOUND: {
        ...
        if (!__wasi_sockets_utils__stream(socket, NULL, &streams, &error)) {
    ...
    udp_borrow_outgoing_datagram_stream_t outgoing_borrow = udp_borrow_outgoing_datagram_stream(streams.outgoing);
    while (true) {
        uint64_t allowed;
        if (!udp_method_outgoing_datagram_stream_check_send(outgoing_borrow, &allowed, &error)) {
            errno = __wasi_sockets_utils__map_error(error);
            return -1;
        }
        ...
        if (allowed) {
            ...
            if (!udp_method_outgoing_datagram_stream_send(outgoing_borrow, &list, &datagrams_sent, &error)) {
                errno = __wasi_sockets_utils__map_error(error);
                return -1;
            }

            assert(datagrams_sent == 0 || datagrams_sent == 1);
        ...
        if (should_block) {
            poll_borrow_pollable_t pollable_borrow = poll_borrow_pollable(streams.outgoing_pollable);
            poll_method_pollable_block(pollable_borrow);
        } else {
            errno = EWOULDBLOCK;
            return -1;
