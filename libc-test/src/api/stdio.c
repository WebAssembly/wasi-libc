...
#ifdef __wasilibc_unmodified_upstream // WASI libc has no ctermid
{char*(*p)(char*) = ctermid;}
#endif
...
#ifdef __wasilibc_unmodified_upstream // WASI libc has no gets
{char*(*p)(char*) = gets;}
#endif
{FILE*(*p)(char**,size_t*) = open_memstream;}
#ifdef __wasilibc_unmodified_upstream // WASI libc has no popen
{int(*p)(FILE*) = pclose;}
#endif
