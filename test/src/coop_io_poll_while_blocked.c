...
int main(void) {
  ...
  // Let the thread get blocked
  for (int i = 0; i < 100; i++)
    TEST(sched_yield() == 0);

  // Polling for write on this thread still works
  struct pollfd pfd = {.fd = conn_fd, .events = POLLWRNORM, .revents = 0};
  TEST(poll(&pfd, 1, -1) == 1);
  TEST(pfd.revents & POLLWRNORM);

  // Concurrent reads don't work.
  char buf[16];
