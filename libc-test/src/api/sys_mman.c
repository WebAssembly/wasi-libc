...
#ifdef __wasilibc_unmodified_upstream // WASI has no mlock
{int(*p)(const void*,size_t) = mlock;}
{int(*p)(int) = mlockall;}
#endif
{void*(*p)(void*,size_t,int,int,int,off_t) = mmap;}
#ifdef __wasilibc_unmodified_upstream // WASI has no mprotect
...
#ifdef __wasilibc_unmodified_upstream // WASI has no posix_madvise
